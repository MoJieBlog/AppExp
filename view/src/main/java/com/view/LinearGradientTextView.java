...
public class LinearGradientTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    public void verticalGradient(final int startColor, final int endColor, final boolean top2Bottom) {
        post(new Runnable() {
            ...
            public void run() {
                if (top2Bottom){
                    linearGradient = new LinearGradient(0, 0, 0, viewHeight, startColor, endColor, TileMode.CLAMP);
                }else{
                    linearGradient = new LinearGradient(viewHeight, 0, 0, 0, startColor, endColor, TileMode.CLAMP);
                }
    ...
    public void horizontalGradient(final int startColor, final int endColor,final boolean left2Right) {
       post(new Runnable() {
           ...
           public void run() {
               ...
               if (left2Right) {
                   linearGradient = new LinearGradient(0, 0, viewWidth, 0, startColor, endColor, TileMode.CLAMP);
               } else {
                   linearGradient = new LinearGradient(viewWidth, 0, 0, 0, startColor, endColor, TileMode.CLAMP);
               }
               paint.setShader(linearGradient);
               postInvalidate();
           }
    ...
    public void setGradientColor(int startX, int startY, int endX, int endY, int startColor, int endColor) {
        linearGradient = new LinearGradient(startX, startY, endX, endY, startColor, endColor, TileMode.CLAMP);
        paint.setShader(linearGradient);
        postInvalidate();
    }
